...
def Transfer_cost(v,action):
  return(torch.tensor(torch.sum(v)-v[action]))
...
class Agent:
    def __init__(self, state_size, action_size, model, device, is_eval=False, model_name=''):
        ...
        self.model = model.double().to(self.device) if is_eval else DQN(self.state_size, self.action_size).double().to(self.device) #if in test mode, self.model = model, else: self.model=DQN()
    ...
    def expReplay(self, batch_size, update_target_net_frequency, loss_function, optimizer):
        ...
        for state, action, reward, next_state, done in mini_batch:
            state, action, reward, next_state, done = torch.tensor(state).double().to(self.device), torch.tensor(action).to(self.device), torch.tensor(reward).to(self.device), torch.tensor(next_state).double().to(self.device), torch.tensor(done).to(self.device)
            target = reward
...
def take_action(iteration, s0,agent):  
...
def record_transition_data_experience_replay(action,s, next_state,alpha,queue_load_index,parent_load_index,rewards,alpha_std,transfer_costs,agent,batch_size,mse,optimizer,batch_losses,done,transitions):
  ...
  transitions.append((s, action, reward, next_state, done)) 
...
def read_line(line):
  ...
  return torch.tensor(str_to_float[0:16])
  


def read_state(s,action,alpha,queue_load_index,parent_load_index,rewards,alpha_std,transfer_costs,agent,batch_size,mse,optimizer,batch_losses,transitions):
  while 1:
    ...
    with open(file_path, "r") as f:
      ...
      if len(lines) > 0 and lines[0] == "DONE":
        ...
        next_state=s
        record_transition_data_experience_replay(action,s,next_state,alpha,queue_load_index,parent_load_index,rewards,alpha_std,transfer_costs,agent,batch_size,mse,optimizer,batch_losses,done,transitions)
        return 1
      elif len(lines) > 0 and lines[-1] == "STATE_READY\n":
        #print (lines)
        done=False
        next_state=read_line(lines[-2])  
        #print('recorded next_state is: {}'.format(next_state))              
        record_transition_data_experience_replay(action,s,next_state,alpha,queue_load_index,parent_load_index,rewards,alpha_std,transfer_costs,agent,batch_size,mse,optimizer,batch_losses,done,transitions)
        return 0
      else:
        continue
...
def write_action(action,file_path):
...
def main(s0,file_path,alpha,queue_load_index,parent_load_index,rewards,alpha_std,transfer_costs,agent,batch_size,mse,optimizer,batch_losses):
  iteration=0  
  transitions=[]
  while 1:
    action, s = take_action(iteration, s0,agent)
    rtn = read_state(s,action,alpha,queue_load_index,parent_load_index,rewards,alpha_std,transfer_costs,agent,batch_size,mse,optimizer,batch_losses,transitions)
    ...
    write_action(action,file_path)
    iteration+=1
    print('iteration:{}'.format(iteration))
    print('gonna save')
    torch.save(batch_losses, 'loss.pt')
...
if __name__ == "__main__":
  main(s0,file_path,alpha,queue_load_index,parent_load_index,rewards,alpha_std,transfer_costs,agent,batch_size,mse,optimizer,batch_losses)
