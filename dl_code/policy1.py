...
class Agent:
    def __init__(self, state_size, action_size, model, device, is_eval=False, model_name=''):
        ...
        self.memory = deque(maxlen=10000) #increase memory
        self.device=device
...
def record_transition_data_experience_replay(action,s, next_state,done):
  reward=-(alpha*np.std(next_state.iloc[:, 0:16].iloc[:, queue_load_index]) + Transfer_cost(s.iloc[:, 0:16].iloc[:, parent_load_index], action))
  #the reward is the std of the queue loads of the next state and the transfer cost
  rewards.append(reward)
  alpha_std.append(alpha*np.std(next_state.iloc[:, 0:16].iloc[:, queue_load_index]))
  transfer_costs.append(Transfer_cost(s.iloc[:, 0:16].iloc[:, parent_load_index], action))          
