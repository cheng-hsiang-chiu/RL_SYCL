...
#
# We generate tasks level by level. Each level has [4, 7) tasks. 
# Each task has at most 3 successors. 
#
...
def generate(output_file, num_tasks):
  ...
  # write edges
  num_tasks_this_level = random.randrange(4, 7, 1)
  num_tasks_next_level = random.randrange(4, 7, 1)
  tid = 0
  cnt_edges = 0
  cnt_tasks = 0
  with open(output_file, "a") as f:
    while tid < num_tasks:
      ...
      cnt_edges = random.randrange(2, 4, 1)
...
if __name__ == "__main__":
  if len(sys.argv) > 1:
    ...
    if (num_tasks < 7):
      sys.exit("Number of Tasks must be greater than 7")
  
  # generate 50 tasks default 
  else:    
    num_tasks = 50 
